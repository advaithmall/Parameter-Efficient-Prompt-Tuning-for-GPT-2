...
stories = torch.load("fin_in.pt")
summaries = torch.load("fin_out.pt")
...
import rouge
n_epochs = 100
from model_trans import Prompt_GPT
...
for epoch in range(n_epochs):
    ...
    for i, (stor, summ) in enumerate(val_loader):
        ...
        if i%1 ==0:
            clear_cuda_memory()
            clear_cuda_memory()
    torch.save(model, "prompt_gpt_trans.pt")
    torch.save(train_loss_list, "train_list5_trans.pt")
    torch.save(val_los_list, "val_list_trans.pt")
    torch.save(rouge_list, "rouge_list_trans.pt")
    print("Model Saved")
    scheduler.step()
    clear_cuda_memory()
    clear_cuda_memory()
